...
class MessageManager:
    ...
    @classmethod
    def create_docs_from_message(cls, msg_json):
        """ create docs from the message passed"""
        document_list = list()
        msg = json.loads(msg_json)
        for msgdoc in msg['documents']:
            logger.debug(msgdoc['type'])
            form_def = FormDefinition.find_one_by_third_party_type(msgdoc['type'])
            if form_def is None:
                raise ValueError(_(__name__ + ".exceptions.third_party_form_does_not_exist"))
            document = Document(form_id=form_def.id)
            if document is None:
                raise ValueError(_(__name__ + ".exceptions.document_could_not_be_created"))
            ...
            for field in msgdoc['fields']:
                for key, value in field.items():
                    document.set_field_value(document.get_field_id_by_messagemap(key), value)
